...
```
  ...
  **First, you could group_by vegetable and variety in the garden_harvest dataset. Then you would mutate weight into wt_lbs. Then you would summarize the sum of the weights for each vegetable (named sum_wt_lbs). Then, you would left_join price_w_tax from the supply_costs dataset. You would end up with a table that has the variables vegetable, variety, sum_wt_lbs, and price w tax. Then you could manually calculate and compare the prices you paid for the amount (lbs) of vegetables you harvested vs the price you would have paid for the same amount (lbs) of vegetables at a Whole Foods.**
...
garden_harvest %>% 
  ...
  mutate(wt_lbs = weight*0.00220462) %>% 
  group_by(variety) %>% 
  summarize(min_date = min(date), sum_wt = sum(wt_lbs)) %>% 
  arrange(min_date) %>% 
...
garden_harvest %>% 
  ...
  distinct(vegetable, variety, has_er_or_ar) %>% 
...
```{r}
Trips %>%
  mutate(hour = hour(sdate), minute = minute(sdate),
 ...
 **On saturday and sunday, there is more activity during the middle of the day, vs the weekdays have a lull in business during that same time**
...
Trips %>%
  mutate(hour = hour(sdate), minute = minute(sdate),
         time_of_day = hour + minute/60,
         day = weekdays(sdate)) %>% 
  ggplot(aes(x= time_of_day, fill = client, alpha = .5))+
...
Trips %>% 
  mutate(hour = hour(sdate), minute = minute(sdate),
         time_of_day = hour(sdate) + minute/60,
      week_day = wday(sdate, label = TRUE),
      weekend = ifelse(week_day == c("Sat", "Sun"),   "Weekend","Weekday" )) %>% 
  ggplot(aes(x = time_of_day, fill = client, alpha = .5))+
  geom_density(color = NA)+
...
Trips %>% 
  mutate(hour = hour(sdate), minute = minute(sdate),
      ...
      week_day = wday(sdate, label = TRUE),
      weekend = ifelse(week_day == c("Sat", "Sun"),   "Weekend","Weekday" )) %>% 
  ggplot(aes(x = time_of_day, fill = week_day, alpha = .5))+
  geom_density(color = NA)+
  facet_wrap(vars(client))
```
  
 ** This graph tells us the specific day in which casual/registered renters rent a bike, where the previous graph gave us general 'weekend' and 'weekday'. The second graph is a bit hard to read since the colors are all muddled together, but I think it depends on how specific you want your data to be presented. I would use both of these graphs, but perhaps for different questions.**
...
Stations %>% 
  ...
  summarize(count = n()) %>% 
  ggplot(aes(x = lat, y = long, color = count, size = count))+
  geom_point()
...
```
  ...
  **I'm sorry, I didn't know how to do #18, so this question tripped me up as well.**
